...
工作目录通过只读进程内存获取，当前支持 64 位 Windows 工具箱读取 x64/WOW64 进程；权限不足、进程退出或内部布局不兼容时明确提示，不以可执行文件目录代替。进程可自行改变工作目录，因此显示的是当前值而非保证不变的启动目录。其余详情使用批量 CIM 查询，随查询刷新。当前未增加 CPU/内存监控。
